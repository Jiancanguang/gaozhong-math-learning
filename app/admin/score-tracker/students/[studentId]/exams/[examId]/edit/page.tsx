...
import { deleteExamAction, updateExamAction } from '@/app/admin/score-tracker/actions';
...
import { getExamRecordById, getStudentById, isScoreTrackerTableMissingError } from '@/lib/score-tracker';
...
export default async function EditExamPage({ params, searchParams }: EditExamPageProps) {
  ...
  const gate = renderScoreTrackerAdminGate({
    successPath: `/admin/score-tracker/students/${params.studentId}/exams/${params.examId}/edit`,
    searchError: error
  });
  if (gate) return gate;
  ...
  try {
    [student, record] = await Promise.all([getStudentById(params.studentId), getExamRecordById(params.examId)]);
  } catch (fetchError) {
      ...
      return (
        <div className="mx-auto w-full max-w-5xl px-4 pb-12 pt-8 sm:px-6 lg:px-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-3xl font-semibold text-tide">编辑考试记录</h1>
            </div>
            <AdminLogoutButton redirectPath={`/admin/score-tracker/students/${params.studentId}/exams/${params.examId}/edit`} />
          </div>
          <div className="mt-6">
  ...
  if (!student || !record || record.studentId !== student.id) {
    notFound();
  }
    ...
    <div className="mx-auto w-full max-w-5xl px-4 pb-12 pt-8 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-tide">编辑考试记录</h1>
          <p className="mt-2 text-sm text-ink/70">
            {student.name} · {record.examDate} · {record.examName}
          </p>
        </div>
      ...
      <div className="mt-6">
        <ExamForm
          action={updateExamAction.bind(null, student.id, record.id)}
          submitLabel="保存修改"
          cancelHref={`/admin/score-tracker/students/${student.id}`}
          values={record}
          deleteAction={deleteExamAction.bind(null, student.id, record.id)}
        />
      </div>
    </div>
  );
